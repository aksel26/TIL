...
> 컴포넌트간 데이터가 흐르고 있는 <u>'상태'</u> 를 관리하는 것.

### Vuex 

> 규모가 큰 애플리케이션의 데이터 관리를 효율적으로 할 수 있게 해준다.



### 반응성

> 데이터의 변화가 감지되었을 때 자동으로 화면을 갱신하는 특성

- 인스턴스 생성 시 data속성 정의 객체들은 내장 API를 통해 getter, setter 형태로 변환된다.
- watcher
...
> 서버사이드렌더링 Vs 클라이언트 사이드 렌더링
>
> > ​	서버 사이드 렌더링 : 서버 사이드에서 완벽하게 코드 해석 후 화면에 표시해 주는 것.
> >
> > ​	클라이언트 사이드 렌더링 : '완성된 후'가 아닌 필요할 때마다 JS를 이용해 나머지 부분을 그리는 것.

- 서버사이드 렌더링의 장점 : 검색엔진에 최적화 (노출정도가 높기 때문)

- 클라이언트 사이드 렌더링 장점 : 초기화면 렌더링 속도, UX
...
> 모듈 번들러
>
> > HTML, CSS 의 파일을 하나의 JS로 묶자.
> >
> > 왜 ? 속도가 더 빠르기 때문

- HTML, CSS, JS 하나씩 불러오기 때문에 그만큼 시간 소요가 필요하게 된다. -> 하나로 묶는게 유리하다.
...
- 인메모리 기반
  ...
  - `npm run build` : 빌드, 이 명령어를 실행하게 되면 /dist/라는 위치에 빌드된다.
  - 하지만, `npm run dev` 명령을 있지도 않은 `/dist` 를 참조하지 않았는데도 빌드된 것처럼 실행된다.
  - 왜냐하면 빌드파일을 메모리에 저장하고 있기 때문에 가능하다. --> **인메모리**
