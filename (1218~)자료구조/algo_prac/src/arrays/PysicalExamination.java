...
//	
//	추상 클래스 (abstract)
//		: 불완전한 클래스로 인스턴스를 만들 수 없다
//			추상메서드 : 실체가 정의되지 않은 메서드, 서브클래스에서 실체를 정의한다.
//		
//	중첩클래스
//		: 멤버 클래스 : 그 선언이 다른 클래스 또는 인터페이스 선언에 둘러싸인 클래스
//		  내부 클래스 : 명시적, 암묵적으로도 정적(static) 으로 선언되지 않는 중첩클래스. 정적 초기화나 멤버 인터페이스 선언을 할 수 없다. 
//		  지역 클래스 : 이름이 주어진 중첩클래스인 내부 클래스. 어떤 클래스 멤버도 될 수 없다. 
