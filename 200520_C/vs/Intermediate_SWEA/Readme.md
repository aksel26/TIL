...
- 본문이 되는 String에서 **특정한 String**을 패턴이라고도 한다.

- 패턴을 찾는 방법을 일컬어 패턴 매칭이라고 한다.

  1. BruteForce 

     하나씩 순서대로 비교한다.

     비효율적 ( 시간 복잡도 : `O(n(텍스트 길이)*m(패턴의 길이))` )
  ...
  2. KMP
     ...
     - 알고리즘에서 사용되는 용어
       ...
       - **lps 배열** (longest proper prefix which is also suffix )
         - 접두와 접미 각각 비교해 가장 긴(longest) 값을 구한다.
     - 

  3. 보이어-무어

     <img src="Readme.assets/image-20200615161618567.png" alt="image-20200615161618567" width="70%" />

     - 일치하지 않으면 비교 문자열 안에 배열 표대로 칸 수를 이동한다.
       - 일치하는 알파벳이 없으면 5칸(비교 문자열 길이) 만큼 이동,
     - 일치하면 역순으로 하나씩 비교한다.



- 1, 2 ---> 'one', 'two'  로 바꾸는 경우
